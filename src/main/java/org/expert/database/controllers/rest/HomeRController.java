...
public class HomeRController {
    ...
    public ResponseEntity<Map<String, Object>> findExpertBySkillId(@PathVariable("skill_id") int skill_id) {
        HttpEntity<Object> request = new HttpEntity<Object>(header);
        ResponseEntity<Map> response = rest.exchange(WS_URL + 
                "/api/expert/find-by/skill-id/{skillId}?paramSkillId=2",
                HttpMethod.GET, request, Map.class);
        return new ResponseEntity<Map<String, Object>>(response.getBody(), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Map<String, Object>> findExpertBySkill() {
        ...
        String URL = WS_URL + "/expert/find-by/skill-id/{skillId}?paramSkillId=2";
