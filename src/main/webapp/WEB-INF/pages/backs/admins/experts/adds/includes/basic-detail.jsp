<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<div class="row">
    <div class="col-xs-12">                        
        <div class="box">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">Sok Channy</h3>
                </div>
                <div class="box-body">
                    <div class="row">
                        <div class="col-xs-2">
                            <div class="pull-left image">
                                <img src="/resources/static/img/profile/channy.jpg" class="img-responsive" alt="User Image">
                            </div>
                        </div>
                        <div class="col-xs-10">
                            <h4 class="heading">Basic Information</h4>
                            <table class="col-xs-12 text-left table-condensed">
                                <tbody>
                                    <tr>
                                        <th style="width: 20%">Birthday</th>
                                        <td style="width: 70%"><input type="text" value="June 07, 2016" class="edit--text" disabled/></td>
                                        <td>
                                            <a href="#" onclick="edit(this)" data-skin="skin-blue-light" class="btn btn-default btn-xs"><i class="fa fa-edit"></i></a>                                        
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Gender</th>
                                        <td><input type="text" value="Male" class="edit--text" disabled/></td>
                                        <td>
                                            <a href="#" onclick="edit(this)" data-skin="skin-blue-light" class="btn btn-default btn-xs"><i class="fa fa-edit"></i></a>                                        
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Address</th>
                                        <td><input type="text" value="Phnom Penh, Chamka Mon, Cambodia" class="edit--text" disabled/></td>
                                        <td>
                                            <a href="#" onclick="edit(this)" data-skin="skin-blue-light" class="btn btn-default btn-xs"><i class="fa fa-edit"></i></a>                                        
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Occupation</th>
                                        <td><input type="text" value="Web Developer" class="edit--text" disabled/></td>
                                        <td>
                                            <a href="#" onclick="edit(this)" data-skin="skin-blue-light" class="btn btn-default btn-xs"><i class="fa fa-edit"></i></a>                                        
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>Organization/Firm</th>
                                        <td><input type="text" value="KSHRD" class="edit--text" disabled/></td>
                                        <td>
                                            <a href="#" onclick="edit(this)" data-skin="skin-blue-light" class="btn btn-default btn-xs"><i class="fa fa-edit"></i></a>                                        
                                        </td>
                                    </tr>
                                </tbody>
                            </table>          
                        </div>                        
                    </div>
                </div>                
            </div>
        </div>
    </div>
</div>

